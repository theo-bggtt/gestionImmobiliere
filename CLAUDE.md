...
Currently at **Étape 0/1/2/3** (socle, capture, retrouver, partager) of a staged build. Full spec, the 8 locked design decisions, and the 12 non-negotiable rules live in `.decisions/implementation-plan.md` — read it before any schema or permission-model change. `README.md`'s "Décisions prises" section lists implementation choices made where the spec was silent.
