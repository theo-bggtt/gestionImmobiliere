...
- npm
- **Un shell POSIX**, parce que toutes les commandes de ce fichier en sont :
  `cp`, `VARIABLE=valeur commande`, `set -a && source`, `openssl rand`, et les
  deux scripts `.sh`. Aucune ne marche dans PowerShell. Le développement se
  faisant sous Windows (décision #142), le shell de référence est **Git Bash**,
  livré avec Git for Windows : il porte `openssl`, il exécute les scripts du
  dépôt, et c'est celui que Claude Code utilise sous Windows. WSL convient
  aussi. En plus, pour `SANS_DOCKER=1 ./scripts/restauration.sh` : `pg_dump` et
  `pg_restore` dans le `PATH` (les outils clients PostgreSQL). Les scripts ne
  sont **pas** portés en PowerShell : deux écritures d'une même procédure
  divergeraient, et c'est la procédure de restauration.
...
L'application tourne sur un **VPS** — une machine virtuelle Linux chez un hébergeur — derrière un nom de domaine et un certificat réels. Une machine, un `docker compose`, un proxy, des sauvegardes — rien d'autre (pas de CI, pas de supervision, pas d'orchestration : la discipline du plan vaut aussi pour l'infrastructure). Ni le fournisseur, ni l'offre, ni le pays d'hébergement ne sont arrêtés (décision #142) : rien de ce qui suit n'en dépend, sauf deux chiffres qui se choisissent **avec** l'offre et qui sont dits là où ils comptent — la taille du disque, parce que le volume des photos grossit sans borne applicative, et la mémoire, parce que l'image se construit sur la machine qui la sert.

Trois choses changent par rapport à une machine posée chez le propriétaire, et elles sont tout le contenu de cette section : **l'adresse est publique**, donc tout port publié est sur Internet et non dans un salon ; **une sauvegarde qui reste sur la VM ne survit pas à la perte de la VM** ; et **le build tourne sur la machine qui sert l'application**.
...
Internet ──443/80──▶ caddy (TLS, redirection HTTP→HTTPS, compression, borne du corps)
                     ...
                     app (node server/app.js, PROXYS_DE_CONFIANCE=1)
                        │ en-têtes de sécurité, CSP à nonce, limite de débit, bornes de taille (411/413)
                        │ puis React Router
                        ▼
...
| `X-Forwarded-*` | Caddy → app | Caddy **remplace** ce qu'un client aurait écrit ; l'app se fie à **un** saut, jamais à `true` (décision #126). Un saut **parce que rien n'est posé devant Caddy** : un load balancer de l'hébergeur ou un proxy Cloudflare en ajouterait un, et `1` deviendrait faux |
...
| Borne de taille des envois | les deux | 413 sur `Content-Length` (Express) et `request_body max_size` (Caddy) — **qui ne borne qu'à la LECTURE**, donc inopérant si aucune route ne lit le corps (#33). Un corps sans longueur annoncée est donc refusé en amont, **411** : Caddy sur l'absence de `Content-Length` (il termine HTTP/2), Express sur `Transfer-Encoding` (il ne voit que HTTP/1.1) |
...
| Journaux | Docker, `json-file` | 3 × 10 Mo par service, rotation : le disque du VPS ne se remplit pas d'un journal |
| Redémarrage | Docker, `restart: unless-stopped` | Au redémarrage de la VM et après un crash, sans unité systemd à écrire |

### Première mise en service

**D'abord la machine, avant de cloner quoi que ce soit.** Un VPS a une adresse publique : il n'y a pas de box devant, et rien de ce qui suit n'était nécessaire quand la machine vivait derrière un routeur domestique.

1. **SSH par clé, et seulement par clé.** La clé publique du PC dans `~/.ssh/authorized_keys`, puis `PasswordAuthentication no` et `PermitRootLogin no` dans `/etc/ssh/sshd_config`, puis `sudo systemctl restart ssh` — sans fermer la session en cours avant d'avoir vérifié qu'une seconde s'ouvre.
2. **Pare-feu : 22, 80, 443, rien d'autre.** Celui de l'hébergeur est le plus sûr, parce qu'il est **en amont de la VM** et qu'une VM compromise ne le désarme pas. `ufw` sur la machine fait l'affaire, à la condition ci-dessous.
3. **`ufw` ne voit pas ce que Docker publie**, et c'est le piège de cette étape. Docker écrit ses propres règles `iptables` dans la chaîne `DOCKER`, traversée **avant** celles d'`ufw` : un service publié sur `0.0.0.0` est joignable depuis Internet pendant qu'`ufw status` affiche « deny ». Ce qui protège la base n'est donc pas le pare-feu, c'est le `127.0.0.1:` devant le port dans `docker-compose.yml` — c'est écrit à côté de la ligne, et `tests/serveur/compose-ports.test.ts` échoue si un service autre que `caddy` publie ailleurs.

Ensuite, avec Docker et Docker Compose installés et le service Docker activé au démarrage (`sudo systemctl enable docker`) :
...
# DOMAINE : le nom qui pointe sur l'adresse publique du VPS (A/AAAA — une
# adresse fixe fournie avec la machine, plus de DNS dynamique à tenir)
...
1. `docker compose up -d --build`, 80 et 443 encore fermés au pare-feu. L'application démarre et applique ses migrations ; Caddy, lui, ne peut pas encore obtenir de certificat et réessaie tout seul. La construction se fait **sur la VM** : compter environ 90 secondes à froid et de l'ordre d'un demi-gigaoctet de mémoire en pointe (mesure du 12 septembre 2026, approximative par construction — voir « Limites connues »).
2. Ouvrir 80 et 443 : au pare-feu de l'hébergeur, et à `ufw` s'il est en place. Caddy obtient le certificat dans la minute (`docker compose logs -f caddy` : « certificate obtained »).
3. **Créer le compte du propriétaire tout de suite**, sur `https://<DOMAINE>/inscription`. L'inscription se ferme sur le premier compte (décision #130) et ne se rouvre que par une **invitation** — un lien à jeton créé depuis `/proprietes/invitations`, qui expire et se révoque (décision #145). Aucune variable d'environnement ne la rouvre : il n'y en a plus. C'est le seul moment où la porte est ouverte sur Internet : entre l'ouverture des ports et cette inscription, quiconque connaîtrait déjà le domaine pourrait s'inscrire à votre place. Si `/inscription` répond « fermées » avant que vous l'ayez fait, ne cherchez pas plus loin : `docker compose down && docker volume rm gestionimmobiliere_postgres_data`, ports refermés, et recommencer. Vérifier ensuite que `/inscription` répond bien « Les inscriptions sont fermées ».
4. Charger le catalogue **depuis un poste de développement**, par un tunnel SSH vers le port 5432 que le compose ne publie que sur la boucle locale du VPS — l'image de production n'embarque pas `tsx`, et c'est voulu : le catalogue se charge une fois, ses alias se rafraîchissent rarement, et un outil de plus dans l'image ne vaut pas ce cas. `--env-file` laisse l'environnement gagner sur `.env`, donc :

   ```bash
   ssh -N -L 5433:127.0.0.1:5432 <utilisateur>@<adresse-du-vps> &
   DATABASE_URL="postgres://gestion:<POSTGRES_PASSWORD du VPS>@127.0.0.1:5433/gestion_immobiliere" npm run seed:catalogue
   ```
...
5. Depuis un poste qui n'est pas la VM — le PC, un téléphone en 4G : sur un VPS, tout le monde est « dehors » —, ouvrir `https://<DOMAINE>/` et constater dans les en-têtes réels, pas dans le code :
...
7. **Constater depuis l'extérieur que rien d'autre ne répond.** C'est la vérification qui n'existait pas quand la machine était derrière une box : ici, il n'y a rien devant. Depuis le PC (PowerShell — c'est un contrôle, pas un script du dépôt) :

```powershell
Test-NetConnection <adresse-du-vps> -Port 443    # TcpTestSucceeded : True
Test-NetConnection <adresse-du-vps> -Port 5432   # attendu : False
Test-NetConnection <adresse-du-vps> -Port 3000   # attendu : False
```

   Un `True` sur 5432 ou 3000 veut dire qu'un `ports:` publie hors de la boucle locale, ou qu'un pare-feu est cru alors qu'il est contourné (point 3 plus haut). Dans les deux cas la base est sur Internet, et la seule réponse est de refermer avant d'aller plus loin.

### Ce que la machine fait toute seule
...
`scripts/sauvegarde.sh` produit deux fichiers horodatés dans `SAUVEGARDES` (défaut `./sauvegardes`) : `<date>.base.dump` (`pg_dump` au format custom, depuis le conteneur) et `<date>.fichiers.tgz` (le volume des photos, `tar` depuis le conteneur de l'application). **La base seule ne restaure rien d'utile** : les chemins sont en base, les octets sur le volume, les deux ne valent qu'ensemble. Rétention locale `RETENTION_JOURS` (défaut 14). Si `SAUVEGARDE_DESTINATION` est renseigné (une cible `rsync` : un NAS, une autre machine), le dossier y est copié à chaque sauvegarde, **sans `--delete`**.

**Ce que l'absence de `--delete` protège, exactement.** Une purge locale — un `RETENTION_JOURS` trop court, un bug du script, un `rm` malheureux — ne se propage pas à la copie distante. C'est réel et c'est ce pour quoi c'est écrit. Ce que ça ne protège **pas** : une machine compromise. La clé SSH qui écrit là-bas écrit, `--delete` ou pas ; qui la tient peut écraser chaque fichier de la destination par du vide. Contre ça il faut autre chose — une destination en **ajout seul**, ou une sauvegarde **tirée** par une autre machine qui se connecte au VPS et jamais l'inverse. Aucune des deux n'est construite ici, et sur une adresse publique la nuance n'est plus théorique.

**Une destination vide n'est pas un défaut acceptable en production.** Une sauvegarde qui ne quitte pas la VM ne survit ni à la perte de la VM, ni à un compte suspendu, ni à un incident chez l'hébergeur — les trois modes de panne d'une machine qu'on ne possède pas, et contre lesquels une copie locale ne peut rien. Le script ne refuse pas pour autant : la sauvegarde locale a bien réussi, et le dire en code de sortie non nul serait faux. Il **avertit**, à chaque passage, sur la sortie d'erreur. Configurer une destination hors machine **puis restaurer depuis elle** est une case de l'issue #25, pas une option.

**Ce qu'un dump contient depuis la liste d'attente.** La table `interesse` porte des adresses e-mail de gens qui **ne sont pas clients** — ils ont laissé une adresse sur une page publique et n'ont rien accepté d'autre. Elles s'ajoutent aux jetons de partage, qui sont en clair dans le dump par construction (un jeton haché ne servirait plus de secret d'URL). Conséquence directe : les **permissions des fichiers de sauvegarde cessent d'être une formalité**. La case `umask 077` de l'issue #25 n'a pas pu être vérifiée le 12 septembre, le dossier de répétition étant sur NTFS, qui ne porte pas les bits POSIX ; elle se vérifie sur le VPS, et elle porte désormais sur des données de tiers. Un dump lisible par tous sur une machine partagée est une fuite d'adresses e-mail, pas seulement une fuite de la maison.

Les **instantanés** de l'hébergeur sont un complément, jamais un remplacement : un instantané pris pendant que Postgres écrit capture un état que seule la reprise après panne de Postgres sait relire, là où `pg_dump` rend un dump cohérent par construction. Et un instantané vit chez le même hébergeur que la VM.

Les deux scripts ne prennent de `.env` qu'une **liste blanche** — `SAUVEGARDES`, `RETENTION_JOURS`, `SAUVEGARDE_DESTINATION`, et `DATABASE_URL` / `STOCKAGE_RACINE` pour le mode `SANS_DOCKER=1`. Ni `SESSION_SECRET` ni `POSTGRES_PASSWORD` n'y figurent : ils ne servent qu'à compose, qui les lit lui-même. L'environnement l'emporte toujours sur `.env`, ce qui permet de restaurer vers une autre base en passant `DATABASE_URL` sur la ligne de commande.
...
```
0 3 * * * cd /home/<utilisateur>/gestionImmobiliere && ./scripts/sauvegarde.sh 2>&1 | logger -t gi-sauvegarde
```

### Restauration

Une sauvegarde jamais restaurée n'est pas une sauvegarde. `scripts/restauration.sh <horodatage>` restaure **dans une base vide et un volume de fichiers**, et refuse une base qui contient déjà des tables — une restauration par-dessus mélangerait deux maisons. Sur un VPS neuf, ou pour repartir de zéro :
...
**Exécutée** le 9 septembre 2026, avec ces deux scripts en mode `SANS_DOCKER=1` (mêmes commandes `pg_dump`/`pg_restore`/`tar`, sans les `docker compose exec`), sur une base de développement portant la « Maison d'exemple » et une photo téléversée par la route de capture, vers une base créée vide et un dossier vide : dix-sept tables comparées au même compte (le schéma en portait déjà vingt à cette date, le chiffre ne pouvait donc être qu'un compte partiel de cette base de développement — la répétition du 12 ci-dessous les compare toutes), la photo et sa vignette présentes au chemin que la base attend, `french_sans_accent` et `unaccent` présents, `plainto_tsquery('french_sans_accent', 'Éclairage')` retrouve l'éclairage, `scripts/migrate.mjs` sur la base restaurée ne fait rien, et une seconde restauration est refusée (« la base n'est pas vide »). La même procédure **avec les vrais `docker compose exec`** a depuis tourné, le 12 septembre 2026, dans la répétition complète sur PC (amd64, `DOMAINE=localhost` ; compte rendu dans l'issue #25) : vingt tables au même compte, les octets au chemin attendu, les déclencheurs restaurés, une seconde restauration refusée. Ce qui reste à faire **sur le VPS** est de la rejouer là-bas, **et depuis la destination hors machine** — c'est elle, et non la copie locale, qui sera la sauvegarde le jour venu. La case correspondante de l'issue #25 reste ouverte jusque-là.
...
- `docker compose up` construit et démarre **sur le VPS**, migrations appliquées depuis une base vide — la construction sur PC est faite (12 septembre, amd64), celle sur la machine qui sert l'application ne l'est pas ; si l'offre retenue est en arm64, c'est là que l'arm64 se prouve ;
...
- une sauvegarde restaurée **sur le VPS**, avec les `docker compose exec`, **et tirée de la destination hors machine** ;
- 5432 et 3000 injoignables depuis l'extérieur, constaté depuis le PC et non déduit du compose (`Test-NetConnection`, § « Première mise en service ») ;
...
- **le refus du corps sans longueur annoncée, côté Caddy** (décision #140). La part Express est tenue par `tests/serveur/`, la part Caddy par rien : aucune suite du dépôt ne lance Caddy. La procédure, à exécuter sur la topologie réelle, est celle qui a trouvé le défaut dans l'issue #33 :

  ```bash
  head -c 41943040 /dev/urandom > gros40.bin

  # Vers une URL dont AUCUNE route ne lit le corps : c'est le cas qui passait.
  curl -k -s -D- -o /dev/null -X POST -H "Transfer-Encoding: chunked" \
    --data-binary @gros40.bin https://localhost/proprietes      # attendu : 411, plus 404

  # Le chemin annoncé, lui, doit continuer de rendre 413 et non 411 :
  curl -k -s -D- -o /dev/null -X POST --data-binary @gros40.bin \
    https://localhost/proprietes                                 # attendu : 413

  # Et rien de légitime ne doit tomber : capture d'une photo depuis le
  # téléphone, et téléversement d'un plan, tous deux jusqu'au bout.
  ```
...
erDiagram
    ...
    PROPRIETE ||--o| COFFRE : "un par propriété"
    ELEMENT ||--o{ SECRET : "codes chiffrés"
```

**`COFFRE` et `SECRET` sont les deux seules tables que le serveur ne sait pas
lire** : la clé de données est enveloppée par une phrase et une clé de secours
qui ne le quittent jamais, et `secret.valeur` est un bloc chiffré par le
navigateur. `SECRET` pend à un objet (comme `GARANTIE`) et n'entre ni dans
`details`, donc ni dans `element.recherche`, ni dans aucune requête de partage.
Voir « Le coffre » et la décision #149.
...
`element.niveau` (0 public · 1 usage · 2 technique · 3 privé) est ce que le plafond d'un lien de partage compare. Il **se saisit sur la fiche**, à la création comme à la modification, et `type_element.niveau_suggere` ne fait que pré-remplir ce choix — la capture, qui ne demande rien, l'écrit tel quel. Voir « Partager ».
...
Ces lignes se remplissent **sur la machine en service, avec la vraie maison** (étape 8, voir [Mise en service](#mise-en-service)) : sur le réseau réel, plusieurs fois, y compris à la cave sans réseau. Les chiffres ci-dessus ont été mesurés sur une machine de développement ; l'écart sera commenté ici, à côté.
...
### Le niveau d'un objet, et d'où il vient

Tout ce qui précède compare `element.niveau` au plafond d'un lien. Pendant huit étapes, **aucun écran ne posait ce niveau** : la colonne valait 3 (privé) par défaut, seul `seed-exemple.ts` écrivait une répartition, et la « Maison d'exemple » a donc masqué le trou jusqu'à la répétition du 12 septembre 2026 (issue #34). Sur une propriété saisie à la main, un lien à plafond 1 rendait « Rien à afficher pour ce lien » : la projection par niveau, qui est la proposition du produit, était inopérante partout sauf dans la démonstration.

Quatre pièces le referment.

**Le catalogue suggère, par type.** `type_element.niveau_suggere` (migration 0010) vaut 1 pour ce qu'un occupant peut avoir besoin de **trouver** — prises, interrupteurs, luminaires, radiateurs, fenêtres, robinets, électroménager, et aussi tableau électrique, disjoncteur, vanne d'arrêt et siphon — et 2 pour ce qu'on ne cherche que pour intervenir : chaudière, chauffe-eau, compteurs, gaine, regard, fosse, pompe à chaleur, cuve, programmateur d'arrosage. Le niveau de la fiche dit si l'objet se **trouve** ; ce qu'on en **apprend** est décidé champ par champ par `niveauMin`, et le calibre d'un disjoncteur comme ce que coupe une vanne y sont déjà en 2. **Aucun type ne suggère 0** : un niveau public se décide objet par objet. C'est le TYPE qui porte la suggestion et non la zone, parce que la portée d'un partage filtre déjà par zone et qu'un tableau électrique posé dans une cuisine doit rester masquable. Le re-seed rafraîchit `niveau_suggere` comme `alias`, jamais `champs` : c'est du vocabulaire de catalogue, qu'un propriétaire ne peut pas corriger sur un type système, et sa correction à lui vit sur la fiche.

**La fiche tranche.** Un sélecteur à la création (pré-rempli par le type) et à la modification (avec la valeur réelle). Un niveau **absent du formulaire est refusé**, pas replié sur 0 : `Number("")` vaut 0, c'est-à-dire « public » — le piège déjà rencontré sur les formulaires d'événement, qui transforme ici un oubli en publication. `lireNiveauSaisi` (`app/lib/partage/niveaux.ts`, neutre) est le seul endroit qui lit un niveau de formulaire.

**La capture ne demande rien de plus** (règle non négociable #8, les 30 secondes) : elle écrit la suggestion du type qu'elle rechargeait déjà pour le valider. Le niveau **ne vient jamais du client** — la boîte d'envoi est de la donnée client, et un `niveau` qui en sortirait serait un choix de visibilité fait par le navigateur ; il se dérive côté serveur, exactement comme le nom. `fichier.niveau` reste à 3 et ne suit pas le type : le droit de lire une photo vient de la fiche.

**Le rattrapage se fait sur l'écran d'une zone** — « mettre les 7 objets de cette Cuisine en usage » —, en deux envois et sans JavaScript : le premier compte ce que ça changerait (combien deviennent plus visibles, combien plus masqués, et pour **chaque lien actif** combien d'objets il gagnerait ou perdrait), le second l'écrit. Cette action ne descend jamais rien à 0, et elle ne touche que la zone visée : une sous-zone a son propre écran. Pas de reprise automatique des objets existants : c'est ce chemin-là, choisi zone par zone, parce qu'un backfill décide à la place du propriétaire ce qu'un tiers verra.

L'aperçu d'un partage **explique sans corriger** : son bandeau dit « 12 objets de cette propriété sont au-dessus du plafond de ce lien », avec un lien vers la zone quand il n'y en a qu'une. Pas de bouton qui monte les niveaux depuis là — c'est exactement l'opération qui fait fuiter, déclenchée à l'endroit où l'on a hâte que quelque chose s'affiche. Ce compte est calculé par le loader de l'aperçu, **jamais** par `chargerContenuPartage` ni rendu par `PagePartage` : les deux loaders sont comparés champ par champ par `tests/partage/routes.test.ts`, et un compte de ce qui est masqué dit qu'il existe des crans au-dessus.
...
### Le tracé survit à son refus

Le tracé ne touche la base qu'à « Terminer le contour » — c'est la lecture la plus simple de « tant que ce n'est pas fini, ce n'est pas enregistré », et elle est inchangée. Ce qui manquait était la suite : l'écran effaçait les clics **au moment de l'envoi**, sans attendre la réponse. Un refus affichait donc son message sur un plan vide, et il fallait tout retracer.

Deux choses le corrigent, et la seconde n'était pas visible depuis l'écran :

- **`traceApres` décide, et c'est une fonction pure** (`app/lib/plans/tracage.ts`), testée sans base ni DOM — même découpage que `regroupement.ts`. Trois issues, une seule efface : rien envoyé ou réponse pas encore là, on garde ; refusée, on garde les clics et on rouvre l'envoi ; acceptée, on efface, parce que le contour est en base. `envoye` dit de quel envoi on attend la réponse : le même fetcher sert aussi à effacer le contour d'une autre zone, et cet effacement n'a aucune raison de jeter des clics en cours. L'écran suit `contours.state` / `contours.data` comme la bannière de proposition suit ceux du fetcher de points — le motif existait déjà dans le fichier.
- **La route rend ses 404 au lieu de les lancer.** Une `Response` **lancée** depuis l'action d'un fetcher ne devient pas `fetcher.data` : elle remonte à la frontière d'erreur et remplace la page entière. Vérifié au navigateur avant de corriger — sur un vrai 404 de couverture, on obtenait un « 404 » nu, et le tracé partait avec le composant qui le tient. Aucune précaution côté écran n'avait de prise dessus. Le code reste 404 sur le fil et le message ne dépend pas du motif : la règle #4 porte sur ce que la réponse **apprend** — « n'existe pas » et « n'est pas à vous » restent indiscernables — pas sur la façon dont le routeur la transporte.

Pendant l'envoi, les sommets restent affichés mais rien ne les modifie : ni clic sur le fond, ni « Annuler le dernier point », ni « Abandonner ». Abandonner à ce moment-là jetterait des clics que le serveur est peut-être en train d'accepter.

Ce qui **ne** change **pas** : changer de plan abandonne toujours le tracé en cours, envoi compris. Les sommets sont des pourcentages de l'*autre* image, et ce qu'on attendrait serait la réponse d'un plan qu'on ne regarde plus.

### L'état d'un contour se lit dans la ligne qu'il décrit

`ZoneTracable.sommets` était chargé (`LEFT JOIN zone_geom` + `jsonb_array_length`), typé et testé, et **jamais lu** : l'écran affichait « 4 points » depuis `polygones`. Il l'est maintenant, et `contourDe` a disparu.

Ce n'est pas un choix esthétique entre deux listes équivalentes. `chargerPolygonesDuPlan` prend une `Portee` et sert aussi les pages de partage ; `chargerZonesTracables` est réservée au propriétaire et n'en prend pas. L'écran passe aujourd'hui `PORTEE_PROPRIETAIRE`, donc le filtre est inerte et les deux listes s'accordent — c'est exactement ce qui rendait le couplage invisible. Le jour où ce loader passerait autre chose (prévisualiser ce que voit un lien est l'envie évidente), « Retracer » redeviendrait « Tracer » sur une zone qui a un contour, et « Tracer » l'écrase : `enregistrerContour` est un `ON CONFLICT DO UPDATE`. Lire le compte dans la ligne qu'il décrit est la seule forme où les deux ne peuvent pas diverger.
...
| **Compte d'objets au-dessus du plafond** | `compterObjetsAuDessusDuPlafond` | **Écran du propriétaire uniquement**, dans le bandeau de l'aperçu. C'est le nombre d'objets que le lien ne voit PAS : le servir dirait qu'il existe des crans au-dessus, comme une tuile « 0 objet » dit qu'il existe une zone. Calculé par le loader de l'aperçu et jamais par `chargerContenuPartage` (les deux sont comparés champ par champ par un test), et `PagePartage` ne le reçoit pas. Même règle pour les comptes du re-nivelage en masse, qui vivent sur l'écran d'une zone. |
| **Niveau suggéré d'un type** | `type_element.niveau_suggere` | **Jamais envoyé.** Métadonnée de visibilité, comme `evenement.niveau` : ce qu'un type propose n'apprend rien d'utile au destinataire et dit qu'il existe des crans. Aucune requête de partage ne la sélectionne. |
...
| **Arbre de la vitrine** (`/`, pages publiques) | *(rien)* | **Ne rend aucune donnée issue de la base**, à aucun plafond : ces pages n'ont ni loader, ni session, ni requête. C'est ce qui leur permet d'être servies en `Cache-Control` public — une page qui lirait la base ne serait pas la même pour tout le monde. Tenu par `tests/vitrine/etancheite.test.ts`, qui suit les imports **en profondeur** depuis chaque module de `app/routes/_vitrine/` et échoue sur tout chemin menant à `app/db/` ou à un `.server`, avec un contrôle qui prouve que le balayage voit une fuite plantée. La tentation a un nom : « déjà 42 personnes inscrites » se lit en base, pas sur la page. |

| **Secret du coffre** | `secret.libelle`, `secret.valeur`, `coffre.*` | **Jamais chargé, à aucun plafond.** Aucune requête sous `app/lib/partage/` ni `app/routes/_partage/` ne nomme `coffre` ou `secret` — `tests/coffre/statique.test.ts` refuse la première occurrence au texte — et aucun type servi à un partage (`FicheRendue`, …) ne porte de champ pour eux, donc les écrire ne compile pas. Le libellé est en clair en base (« Code du portail » dit qu'un code existe, sur l'écran du propriétaire seul) ; la valeur est un bloc chiffré par le navigateur que le serveur ne sait pas ouvrir. `tests/coffre/etancheite.test.ts` balaye toutes les colonnes de toutes les tables après l'enregistrement d'un secret (phrase, clé de secours et valeur portent chacune une sentinelle, contrôle du balayage compris) et sérialise `chargerContenuPartage` à plafond 3, portée entière : ni libellé, ni bloc, ni le mot. |
<p align="right"><a href="#top">↑ haut de page</a></p>
...
- `app/styles/` — trois feuilles faites main, sans outil : `releve.css` (le langage — jetons, les deux `@font-face`, reset, primitives, focus, mouvement de base — servie à **tous** les arbres par `root.tsx`, en premier), `app.css` (les écrans du produit, page d'un lien comprise, servie par `root.tsx` après elle) et `vitrine.css` (les mises en page de vente, servie par le seul layout de la vitrine). `polices/` : Fraunces et Instrument Sans en woff2 latin, licence à côté de chaque fichier ; Vite les hache dans `/assets/`, donc la coquille hors ligne les garde.
...
> **Une décision ne se renumérote jamais, et ne se réécrit pas en silence.** Les
> numéros sont cités depuis le code, depuis les issues et depuis les autres
> décisions : les décaler rendrait faux tout ce qui pointe dessus. Quand une
> décision ultérieure change l'**argument** d'une décision existante — pas son
> issue, son argument —, celle-ci est **amendée en place** et porte la mention
> « (amendée par #N, <date>) » à l'endroit exact qui a bougé, avec ce qui était
> écrit et pourquoi ça ne tient plus. Ce qui a été cru reste lisible ; c'est la
> même forme que les notes de divergence du plan d'implémentation. La
> convention est née avec #142, qui a fait basculer six arguments d'un coup.
...
23. **Une feuille de style a été ajoutée** (`app/styles/app.css`). L'étape 0 n'avait aucun CSS ; des `<select>` natifs non stylés ne tiennent pas un chronomètre de 30 secondes au pouce. Sobre, une seule feuille, cibles à 56 px. Amendée le 2026-09-15 : trois feuilles, voir la décision #144.
...
<details>
<summary><strong>Finitions — 5 décisions (pagination de la chronologie, finitions du tracé de contours)</strong></summary>
<br/>
...
121. **Une `Response` lancée depuis l'action d'un fetcher n'est pas une erreur affichable — elle remplace la page.** Mesuré au navigateur avant de corriger : sur un vrai 404 de couverture (une zone qui change de niveau pendant le tracé, donc un plan qui ne la couvre plus), l'écran de plan était remplacé par un « 404 » nu. Le tracé en cours vit dans ce composant : il partait avec lui, et garder les clics côté écran n'avait aucune prise dessus. `plans/contours` **rend** donc ses 404 au lieu de les relancer, avec le même code de statut et un message qui ne dépend pas du motif — la règle non négociable #4 porte sur ce que la réponse APPREND, « n'existe pas » et « n'est pas à vous » restant indiscernables, pas sur la façon dont le routeur la transporte. `plans/points` garde le comportement lancé : ce qu'une pose de point perdrait est le zoom, pas une saisie. Le jour où elle porterait autre chose, elle prendra la même forme.
...
125. **Caddy comme proxy inverse.** Certificat Let's Encrypt obtenu et renouvelé seul, redirection HTTP→HTTPS implicite, un Caddyfile de quinze lignes, image officielle publiée pour les deux architectures qu'un VPS peut avoir, amd64 et arm64 (**amendée par #142, 13 septembre 2026** : la formulation d'origine disait « image officielle arm64 », parce que la cible était un Raspberry Pi). Écartés : nginx + certbot — deux pièces mobiles et un cron de renouvellement à surveiller, c'est-à-dire exactement ce qu'on ne veut pas administrer sur une machine qu'on regarde deux fois par an (**amendée par #142** : l'argument disait « sur un Pi » ; il ne tenait pas à la petitesse de la machine mais à celle de l'attention qu'on lui donne, et c'est encore plus vrai d'une VM chez un hébergeur, qu'on ne voit jamais) ; Traefik — configuration par étiquettes Docker et tableau de bord, une surface pensée pour des dizaines de services quand il y en a un. Les en-têtes de sécurité ne sont **pas** dans le Caddyfile mais dans Express, où ils se testent et valent aussi sans Caddy ; Caddy ne fait que TLS, la compression, la borne du corps et le refus d'un corps sans longueur annoncée (décision #140).
126. **`trust proxy` vaut `1`, un entier, jamais `true`.** Avec N sauts de confiance, Express lit l'adresse du client à N positions de la fin de `X-Forwarded-For` — là où le dernier proxy digne de confiance l'a écrite. `true` la lirait au **début**, c'est-à-dire là où le client lui-même peut l'écrire, et la limite de débit se contournerait avec un en-tête. Un saut, parce qu'il y a exactement un proxy et que le port de l'application n'est pas publié : le seul pair possible est Caddy, qui par défaut **remplace** les `X-Forwarded-*` entrants par ce qu'il voit lui-même (sans `trusted_proxies`, aucun client n'est cru). La valeur vient de l'environnement (`PROXYS_DE_CONFIANCE`, 1 dans le compose, 0 partout ailleurs) parce qu'elle décrit la machine, pas le code. Ce que ça fait concrètement, et qu'un test constate sur de vraies connexions : `req.ip` vaut l'adresse écrite par Caddy et non celle que le client a mise devant, `req.secure` vaut vrai derrière TLS, donc HSTS part et l'URL vue par React Router est en `https`. **Constaté hors des tests** le 12 septembre 2026, dans la répétition Docker complète : `Strict-Transport-Security: max-age=31536000` sur des réponses réelles derrière Caddy — c'est la preuve que le `X-Forwarded-Proto` du proxy est bien lu, et aucune suite du dépôt ne peut la donner. **La condition, écrite depuis #142 (13 septembre 2026)** : `1` est juste **tant que rien n'est posé devant Caddy**, ce qui était acquis quand la machine était derrière une box domestique et ne l'est plus sur un VPS. Un load balancer de l'hébergeur, un proxy Cloudflare, n'importe quel service qui termine TLS à la place de Caddy ajoute un saut : `1` désignerait alors l'adresse que ce proxy a recopiée du client, c'est-à-dire une adresse choisie par le client — et la limite de débit, dont la clé est `req.ip`, deviendrait un compteur unique partagé par tout Internet. Le jour où quelque chose est mis devant, cette valeur change en même temps, ou elle est fausse. Rien n'est configuré pour ça d'avance : il n'y a rien devant.
...
128. **La limite de débit est un compteur maison, pas `express-rate-limit`.** Fenêtre fixe, clé `req.ip`, mémoire bornée (les fenêtres les plus anciennes partent au-delà de dix mille adresses), horloge injectable, cinquante lignes testées sans HTTP. La bibliothèque ferait la même chose avec un magasin en mémoire, pour une dépendance de plus et un lissage plus fin dont cette application n'a pas besoin (**amendée par #142, 13 septembre 2026** : l'argument disait « dont un Pi n'a pas besoin », et il ne portait déjà pas sur la puissance de la machine — un VPS n'en a pas plus besoin, pour la raison qui suit et qui est la vraie) : ce qu'on protège, c'est le temps de calcul — un argon2 par essai de connexion, une fraction de seconde d'un cœur — et la disponibilité, pas une équité entre clients. Seuils : 10 envois par minute pour connexion et inscription **ensemble** (afficher le formulaire n'est jamais compté), 600 requêtes par 5 minutes sur `/p/` — une page de partage charge une vingtaine de ressources, et une famille derrière une box partage l'adresse. **Jamais sur l'arbre authentifié**, et le test épuise `/p/` et `/connexion` avant de vérifier que vingt-cinq requêtes du propriétaire passent toutes — constaté aussi hors des tests le 12 septembre 2026 : 429 au onzième `POST /connexion`, compteur partagé avec `/inscription`, et trente `GET /` authentifiés d'affilée sans un seul frein. Le 429 est du texte, sous les en-têtes de `/p/`. **Deux précisions ajoutées à la relecture.** La fenêtre est fixe, donc le pic à la bascule est de **2× la borne** — 1 200 requêtes en une milliseconde à cheval sur deux fenêtres, mesuré ; c'était assumé en esprit (« un lissage plus fin dont on n'a pas besoin ») et jamais chiffré. Et la clé n'est plus `req.ip` brute : en IPv6 elle est **tronquée au /64**, parce qu'un abonné résidentiel comme un VPS en obtient un entier — 5 000 requêtes depuis 5 000 adresses d'un même /64 passaient une borne de 600, ce qui vidait de sens la protection du temps de calcul que cette décision invoque.
...
130. **L'inscription se ferme sur le premier compte** (issue #26). Produit à propriétaire unique : `/inscription` sur Internet sans porte était un hébergement de photos ouvert à tous, sur le disque qui porte la maison (**amendée par #142, 13 septembre 2026** : « la carte SD qui porte la maison » — la cible est un VPS, et son disque se remplit de la même façon, en facturant en plus l'espace). Le premier compte s'inscrit librement — c'est le démarrage — et les suivants demandent `AUTORISER_INSCRIPTION=1` dans l'environnement, le temps de créer le compte. Le refus est **uniforme** (« Les inscriptions sont fermées ») que l'adresse existe ou non : porte fermée, on ne regarde même pas l'adresse, et on ne hache rien. Un verrou consultatif sérialise deux premières inscriptions simultanées, un test les lance en parallèle. La page de connexion ne propose plus le lien quand c'est fermé. **Le « temps de créer le compte » est borné en code, et ne l'était pas** : la relecture a mesuré six comptes créés d'affilée en laissant la variable en place. Un opérateur qui oublie de la retirer, ou un `docker compose up -d` qui la relit d'un `.env` jamais nettoyé, rouvrait une inscription publique. Le plafond est désormais **dur, à deux comptes** : la variable laissée en place n'ouvre pas de troisième inscription, elle ne fait plus rien. Deux et pas trois parce que le cas réel du second compte est le conjoint ; au-delà, c'est la ligne « multi-logement » du plan, et elle demandera autre chose qu'une variable d'environnement. Ce que ça coûte le jour où un second compte est légitime : une variable et un redémarrage, puis la retirer — et jusque-là, un « compte existe déjà » ne se dit qu'à qui a ouvert la porte. Écartés : une liste blanche d'adresses (un secret de plus à tenir dans `.env`) et un jeton d'invitation (un écran de plus, pour un cas qui n'est pas arrivé). **Amendée par #145, 16 septembre 2026** : le cas est arrivé, et c'est le jeton d'invitation qui a gagné — `AUTORISER_INSCRIPTION` et son plafond dur n'existent plus. Ce qui avait été écrit ici comme un coût acceptable (« une variable et un redémarrage, puis la retirer ») était en fait le symptôme : un mécanisme sans état a besoin d'un plafond en code pour se protéger de sa propre trace dans un fichier.
131. **Sauvegarde = `pg_dump` + `tar`, et une restauration exécutée avant d'écrire le mot.** Format custom pour la base, archive du volume pour les photos, les deux horodatés ensemble parce que l'un sans l'autre ne restaure rien ; rétention locale de quatorze jours ; copie distante par `rsync` **sans `--delete`** — la rétention distante se fait là-bas. La restauration refuse une base non vide, parce qu'une restauration par-dessus mélangerait deux maisons. Le tout a été **exécuté** (section « Mise en service ») : en mode sans Docker le 9 septembre 2026, puis avec les vrais `docker compose exec` dans la répétition du 12 septembre ; ce qui reste à faire est de le rejouer sur la machine cible, et la case reste ouverte jusque-là. **Deux amendements de #142 (13 septembre 2026), et le premier corrige une affirmation trop forte.** (1) L'absence de `--delete` était écrite « pour qu'un Pi compromis ne puisse pas effacer ce qui le sauve » : c'est faux. Elle protège d'une purge locale — bug du script, rétention trop courte — et de rien d'autre ; la clé SSH qui écrit sur la destination peut y écraser n'importe quel fichier, `--delete` ou pas. Une protection contre une compromission demande une destination en **ajout seul** ou une sauvegarde **tirée** depuis une autre machine, et ni l'une ni l'autre n'est construite ici. Sur une IP publique, l'écart entre ce qui était écrit et ce qui est vrai cesse d'être théorique. (2) « La destination hors machine n'est pas configurée par défaut, et son absence est dite en limite connue » tenait quand la machine était chez le propriétaire, où une sauvegarde locale protège déjà d'une fausse manœuvre. Sur un VPS, une sauvegarde qui reste sur la VM ne survit ni à la perte de la VM, ni à un compte suspendu, ni à un incident chez l'hébergeur. Le script ne refuse toujours pas — la sauvegarde locale a réussi, un code de sortie non nul mentirait — mais il **avertit** désormais à chaque passage sans destination, et « configurer une destination hors machine puis restaurer depuis elle » devient une case de l'issue #25. Les instantanés de l'hébergeur ne remplacent rien : pris pendant que Postgres écrit, ils demandent une reprise après panne là où `pg_dump` est cohérent par construction, et ils vivent chez l'hébergeur de la VM.
132. **`seed:exemple` refuse une base réelle, par TROIS gardes.** `NODE_ENV=production` d'abord — c'est ce que pose le conteneur, donc `docker compose exec app … seed-exemple` est refusé d'office — et l'existence d'un compte qui n'est pas celui de la démonstration ensuite, quel que soit l'environnement. **La troisième a été ajoutée à la relecture, parce que les deux premières laissaient passer le cas qui fait le plus de dégâts** : une base migrée, aucun compte, `NODE_ENV` non défini. C'est l'état d'une instance NEUVE pendant la fenêtre où les ports sont ouverts et le propriétaire pas encore inscrit ; un `npm run seed:exemple` lancé depuis un shell **sur la machine elle-même** à ce moment-là y crée `demo@gestion-immobiliere.local` / `demo1234` — identifiants publiés dans ce fichier — comme **premier** compte, ce qui referme l'inscription contre le propriétaire et laisse le seul compte de l'instance à qui sait lire le dépôt. Le raisonnement écarté était « le port 5432 ne quitte pas la boucle locale, donc *depuis un autre poste avec `NODE_ENV` vide* n'existe pas » : vrai depuis un autre poste, **faux depuis la machine elle-même**, dont on a un shell (**amendée par #142, 13 septembre 2026** : le contre-exemple était « le shell du Pi » ; il vaut mot pour mot pour le shell SSH d'un VPS, qui est le seul moyen d'y faire quoi que ce soit — c'est-à-dire que le cas est plus probable, pas moins). La troisième garde exige donc `SEED_EXEMPLE=1`, un geste explicite, et seulement quand la base n'a pas un seul compte — une fois la démonstration chargée, elle se tait. Les trois sont testées en tant que script réel, comme l'idempotence.
...
<details>
<summary><strong>Issue #34 — 4 décisions (le niveau d'un objet se saisit)</strong></summary>
<br/>

134. **Le niveau suggéré est porté par le TYPE, pas par la zone.** `element.niveau` n'était posé par aucun écran, donc toute propriété saisie à la main restait entièrement en 3 et un lien sous le plafond 3 n'y montrait rien (trouvé pendant la répétition Docker, sur une propriété réelle au lieu de la « Maison d'exemple », qui masquait le trou parce que son seed écrit une répartition). La suggestion va sur `type_element.niveau_suggere` parce que c'est le type qui rend un objet technique : dans la répartition de `seed-exemple.ts`, référence de fait, prises, interrupteurs, radiateurs, fenêtres, robinets et luminaires sont en usage, tableau, chaudière, compteurs, vanne d'arrêt, gaine et siphon en technique, et il n'y a quasiment pas d'exception. Écarté : un niveau porté par la **zone** — la portée d'un partage filtre déjà par zone, et un tableau électrique dans une cuisine doit rester masquable ; ce serait deux filtres pour une même idée, dont l'un mentirait. **Aucun des 33 types ne suggère 0** : un niveau public se décide objet par objet, jamais pour toute une famille. La suggestion ne se propage jamais aux objets déjà posés — elle pré-remplit un formulaire, elle ne décide rien ; `element.niveau` reste ce que lit le filtre, et lui seul (règle non négociable #1 par analogie : ce que lit le filtre ne se réécrit pas tout seul).
135. **Le re-nivelage en masse vit sur l'écran d'une zone, et chiffre avant d'écrire.** « Tous les objets de la Cuisine en usage », en deux envois sans JavaScript : le premier compte (combien deviennent plus visibles, combien plus masqués, et pour chaque lien actif combien d'objets il gagnerait ou perdrait), le second applique. C'est l'écran du propriétaire, donc compter n'y divulgue rien, et c'est le seul endroit d'où la conséquence se voit avant d'être produite — un lien de partage est envoyé par WhatsApp, il ne se rattrape pas. L'action ne descend jamais rien à 0 et ne touche que la zone visée : une sous-zone (Jardin > Potager) est une zone à part, avec son propre écran. Écarté : un **backfill automatique** des objets existants à la migration — il déciderait à la place du propriétaire ce qu'un tiers voit, sur une base qu'il n'a pas relue. Le chemin est l'action, zone par zone, et la migration ne change aucun `element.niveau`.
136. **L'aperçu d'un partage explique, il ne corrige pas.** Son bandeau annonce « N objets sont au-dessus du plafond de ce lien », avec un lien vers la zone concernée quand il n'y en a qu'une. Pas de bouton « ouvrir ces objets » : c'est l'opération qui fait fuiter, offerte là où l'on a hâte que quelque chose s'affiche, et un clic ne se relit pas. Le compte est calculé par le loader de l'**aperçu** et jamais par `chargerContenuPartage` — qui est le loader réel de `/p/:jeton`, dont `tests/partage/routes.test.ts` compare la sortie champ par champ avec celle de l'aperçu. La contrainte a été prise comme une aide et non comme une gêne : ce qui ne peut pas descendre dans les données partagées ne peut pas fuir, et un compte de ce qui est masqué dit qu'il existe des crans au-dessus (même famille que la tuile « 0 objet »).
137. **Le re-seed du catalogue rafraîchit `niveau_suggere`, comme `alias` et jamais comme `champs`.** Un type système n'est pas éditable : personne n'a de correction à écraser sur cette colonne, et celle du propriétaire vit sur la fiche (`element.niveau`), que le rafraîchissement ne touche pas. `champs` reste hors d'atteinte pour la raison inverse — un champ retiré du catalogue doit être masqué, jamais effacé (règle non négociable #5). Pour un type **perso**, le sélecteur propose « technique » (le cas courant : ce que le catalogue n'a pas est en général de l'équipement qu'on ne manipule pas au quotidien) et un formulaire amputé du champ retombe sur 3, le côté fermé — l'inverse du repli sur 0 refusé sur la fiche. Un type perso ne peut pas suggérer 0 non plus.

</details>

<details>
<summary><strong>Issue #38 — 1 décision (ce que le niveau d'un type décide, et ce qu'il ne décide pas)</strong></summary>
<br/>

138. **Le niveau d'une fiche dit si quelqu'un peut avoir besoin de TROUVER l'objet ; les `niveauMin` de ses champs disent ce qu'il en APPREND.** Amendement à **#134**, qui disait d'où vient la suggestion sans dire comment on la choisit — et la dérivation depuis `seed-exemple.ts` a rangé en « technique » quatre types dont la recherche est justement le geste d'un occupant : `Vanne d'arrêt`, `Disjoncteur`, `Tableau électrique` et `Siphon`, tous passés à 1. La vanne d'arrêt est l'exemple fondateur du projet, et le catalogue la masquait au locataire ; le disjoncteur a hérité du niveau de son voisin le tableau, et le tableau suit par cohérence — des disjoncteurs visibles sous un tableau invisible produisent un locataire qui sait qu'un disjoncteur existe et ne sait pas où aller. Vérifié type par type avant d'écrire la règle : `calibre` et `type` d'un disjoncteur, `nombre_disjoncteurs` d'un tableau, `coupe_quoi` d'une vanne, `diametre` d'un siphon sont **déjà** en `niveauMin` 2, donc la fiche descend en usage sans que rien de technique ne sorte. Ce qui reste lisible à 1 est ce qu'on cherchait : le circuit que protège un disjoncteur, le réseau que coupe une vanne, l'emplacement d'un siphon. Corollaire qui verrouille l'amendement dans le bon sens : `champs` n'est **jamais** rafraîchi par le re-seed (#137, règle non négociable #5), donc relever un `niveauMin` ne serait pas une correction de catalogue mais une migration — raison de plus pour que le tri des types ne serve jamais à rattraper un champ mal classé.

</details>

<details>
<summary><strong>Issue #35 — 1 décision (ce que les scripts lisent de `.env`)</strong></summary>
<br/>

139. **Les scripts de sauvegarde lisent `.env` par une liste blanche, pas par un analyseur concurrent de celui de compose.** `sauvegarde.sh` et `restauration.sh` exportaient **tout** `.env` dans leur environnement, ligne par ligne et telle quelle. Or les deux démarrent des conteneurs, et l'environnement gagne aussi sur `.env` pour l'interpolation de compose : ce que le script avait lu remplaçait, pour ces conteneurs-là, ce que compose aurait lu — et les deux analyseurs divergent, compose retirant les guillemets et le `\r` final là où `read` les garde. Mesuré le 12 septembre (issue #35) : `SESSION_SECRET` à 65 caractères côté script contre 64 côté compose, donc **toutes les sessions rejetées par une restauration** alors que la table `session` était bien restaurée et que ce README promet l'inverse. La variante grave n'a pas été tirée par chance : `restauration.sh` initialise `postgres` sur un volume **vide**, donc un `POSTGRES_PASSWORD` divergent aurait fixé le mot de passe du rôle sur une valeur qu'aucun `docker compose up` ultérieur n'aurait présentée — l'instance restaurée ne pouvant alors plus se connecter à sa propre base. C'est ce cas-là qui décide de la forme du correctif : un `tr -d '\r'` ne fermerait qu'une des deux divergences, et seulement pour les valeurs qui continueraient de passer par le script. La liste blanche (`scripts/lire-env.sh`, un `lire_env "NOM NOM"` par script) règle le problème **à la racine** : un secret qui ne transite plus par l'environnement du script ne peut plus diverger de ce que compose lit. Ce qui reste — un dossier, un entier, une cible rsync, et `DATABASE_URL`/`STOCKAGE_RACINE` en mode `SANS_DOCKER=1` — est nettoyé du `\r` et d'une paire de guillemets encadrants, ce que compose fait aussi. `POSTGRES_USER` et `POSTGRES_DB` n'y sont **pas**, vérification faite : ils ne sont lus que **dans** le conteneur, par des `sh -c` en quotes simples. **La priorité « l'environnement gagne sur `.env` » est conservée** — c'est elle qui permet de restaurer vers une autre base en passant `DATABASE_URL`. Écarté : réimplémenter l'analyseur de compose (interpolation, échappements) — il faudrait le suivre, et ce qui reste lu n'est plus un secret, donc une divergence s'y verrait au lieu de se taire.

</details>

<details>
<summary><strong>Issue #33 — 1 décision (le corps sans longueur annoncée)</strong></summary>
<br/>

140. **Un corps qui n'annonce pas sa longueur est refusé en 411, des deux côtés — et les deux côtés ne testent pas la même chose, parce qu'ils ne voient pas le même protocole.** `request_body max_size` de Caddy n'a jamais borné que la **lecture** : il enveloppe le corps dans un lecteur borné, donc l'erreur ne survient qu'au moment où quelqu'un lit. Avec une longueur annoncée, Caddy refuse d'emblée ; sans elle, si le gestionnaire en aval ne lit pas le corps — n'importe quel 404, n'importe quelle route sans action — personne ne lit, donc personne ne borne. Mesuré pendant la répétition Docker : 40 Mo en `chunked` vers `/proprietes` traversaient Caddy **et** Express pour rendre un 404. Trois endroits affirmaient l'inverse (le `Caddyfile`, ce README, `CLAUDE.md`) ; **une documentation qui promet une protection inexistante est pire que l'absence de protection**, et c'était la partie obligatoire. Fermé plutôt qu'assumé : ces octets ne sont lus par personne, mais « personne ne les lit » n'est pas une borne, et la ligne coûte deux directives. **Caddy teste l'absence de `Content-Length`** parce qu'il termine HTTP/2, où `Transfer-Encoding` n'existe pas et où un corps de longueur inconnue n'a aucun en-tête à montrer non plus — viser `chunked` fermerait HTTP/1.1 et laisserait la même porte un étage plus haut. **`server/application.js` teste `Transfer-Encoding`** parce que `node:http` ne parle que HTTP/1.1, où un corps sans longueur *est* un `chunked` : sans l'un ni l'autre en-tête la requête n'a pas de corps du tout (vérifié au socket, les octets qui suivent sont ignorés par Node), donc le critère est exact et un POST sans corps n'est pas refusé au passage. Il est posé là aussi, et pas seulement chez Caddy, pour la même raison que le 413 : c'est la seule borne qui vaille quand on n'est pas derrière le proxy — `npm run dev`, réseau local, port publié par erreur — et c'est la seule des deux que la suite puisse éprouver. `close` côté Caddy et `req.destroy()` côté Express coupent la connexion après la réponse, sans quoi le serveur draine le corps au lieu de fermer. **Rien de légitime n'entre là-dedans, mesuré au navigateur avant d'être écrit** (Chromium piloté, HTTP/1.1 *et* HTTP/2 sur TLS) : la boîte d'envoi de la capture (`fetch` + `FormData` avec une photo), un POST de formulaire natif, le téléversement de plan et même un POST sans corps annoncent tous leur longueur — `Content-Length: 0` dans le dernier cas. Seul un corps en `ReadableStream` s'en passe, et l'application n'en envoie aucun. Ce qui reste non vérifié depuis le dépôt, et le reste : la part **Caddy**, qu'aucune suite ici ne peut lancer.
...
<details>
<summary><strong>Issue #44 — 1 décision (la cible de production est un VPS)</strong></summary>
<br/>

142. **Pas de Raspberry Pi : la production tourne sur un VPS, et le développement sous Windows.** Décidé le 10 septembre 2026. Un VPS est une machine virtuelle Linux chez un hébergeur, avec exactement la topologie de l'étape 8 — `docker compose`, Caddy devant, le volume des photos, `pg_dump` : rien du travail livré n'est jeté, c'est la machine qui change et pas l'architecture. **Écarté, le PaaS** (une plateforme qui construit et fait tourner l'application à partir du dépôt) : il faudrait un stockage objet à la place du volume `fichiers_data`, une base gérée à la place du conteneur `postgres` — donc renoncer à `pg_dump` + `tar` en un script, à la restauration éprouvée, et au « une machine, un compose » que l'étape 8 a écrit et mesuré. Le prix serait payé pour éviter une administration système qui tient en trois points (SSH par clé, pare-feu, `docker compose up`). **Écarté aussi, l'auto-hébergement** : un Pi chez le propriétaire, c'est une adresse résidentielle, un DNS dynamique, une alimentation, une carte SD et une box à ouvrir — cinq pièces mobiles pour une machine qu'on ne veut pas administrer, quand un VPS en donne une seule qui redémarre toute seule. **Ce qui n'est pas décidé et ne l'est pas par cette décision** : le fournisseur, l'offre (disque, mémoire, pays), et l'hébergement en Suisse — ni oui ni non, c'est dans « À vérifier avant de s'appuyer dessus » du plan, avec son déclencheur. **Ce que le changement de cible a réellement coûté** : six décisions dont l'argument nommait le Pi, amendées en place (#67, #125, #128, #130, #131, #132), la condition de `PROXYS_DE_CONFIANCE: 1` enfin écrite (#126), une affirmation fausse corrigée (#131, l'absence de `--delete` ne protège pas d'une compromission), un avertissement ajouté au script de sauvegarde, et un piège de plus dans le runbook — **Docker contourne `ufw`**, donc ce qui tient la base hors d'Internet est le `127.0.0.1:` du compose et rien d'autre. C'est ce dernier point qui justifie à lui seul la relecture plutôt qu'un rechercher-remplacer : derrière une box, personne ne pouvait atteindre le 5432 même si le compose l'avait publié.

143. **La vitrine vend, et n'avance toujours rien que le code ne tienne.** Les cinq pages de #53 étaient écrites sobres, presque en retrait, et c'était voulu à l'étape de la frontière ; elles sont réécrites pour vendre : une entrée par page avec une promesse et un appel, les trois gestes, les quatre lecteurs avec leur échelle de niveaux, le document contre le lien dans un tableau, une bande verte pour la transmission. **Les contraintes de #53 n'ont pas bougé** : aucun chiffre non mesuré, aucune affirmation juridique, aucun prix, aucune image de la vraie maison, jamais le mot proscrit, et `tests/vitrine/discours.test.ts` les tient toujours, sur des pages qui ont trois fois plus de texte à tenter. Ce que ça a coûté : **une police de titrage, Fraunces, servie depuis `public/polices/`** (latin seul, 67 Ko, OFL, licence à côté du fichier), donc `font-src 'self'` dans la politique de la vitrine, seule directive ajoutée ; le corps reste en police système, comme l'application. Pas de police chez un tiers : `/confidentialite` l'affirme, et une police chargée chez Google ferait mentir la page qui vend la confidentialité. Écartés : une seconde police pour le corps (des kilo-octets pour un texte que la police système rend aussi bien) et un italique (le fichier ne porte que le droit, un italique synthétisé fait brouillon : le mot souligné d'un titre passe en vert). Le schéma de l'accueil est un SVG écrit dans la page, chaque trait une classe et jamais un attribut `style`, parce que la politique n'a pas d'`'unsafe-inline'` ; le test qui interdit `<img>` sur la vitrine reste vrai, et l'illustration n'est la maison de personne. L'appel principal reste « Ouvrir mon espace » : l'appel du visiteur qui n'a pas de compte est la liste d'attente (#55), et elle arrive par sa propre PR, dans la carte de fin de l'accueil qui l'attend. La seconde police écartée ici a été adoptée le 2026-09-15 pour tout le produit, et Fraunces a suivi sous `app/styles/polices/` : voir la décision #144.
144. **Le relevé, de la vitrine à la fiche.** Le projet n'avait aucun framework de style, mais deux langages : le relevé d'architecte sur la vitrine (#143) ; la police système, les cartes blanches arrondies et les capsules dans l'application. Décision du 2026-09-15 : **un seul système de design fait main pour les deux**, React Router v7 inchangé. Trois feuilles (`releve.css` déclare une fois les jetons, les deux polices et les primitives ; `app.css` et `vitrine.css` composent avec et n'en redéclarent aucune) ; deux polices auto-hébergées (Fraunces pour ce qui titre, Instrument Sans pour le corps) ; trois traits et jamais un quatrième — la coupe de 2,5 px qui ouvre un bloc, la projection de 1 px entre deux lignes, le tireté de ce qui existe mais n'est pas visible d'où l'on se tient : le modèle de permission devenu vocabulaire de chaque écran ; des angles droits, aucune ombre hors le voile de la feuille et le halo d'un point ; une marge de navigation à gauche à partir de 1000 px, rendue par le layout sans requête ; sept mouvements sobres, tous sous `prefers-reduced-motion`, l'état de repos étant toujours l'état final ; la vitrine réécrite du côté du client, avec des pictogrammes en SVG inline. **Ce que ça renverse** : la clause « écartée : une seconde police pour le corps » de #143, et la clause « police système » de la décision #8 du plan d'implémentation (« style sobre »). **Ce que ça a coûté** : 30 Ko de police de plus (Instrument Sans, latin seul, OFL, sous `app/styles/polices/` avec Fraunces), `font-src 'self'` sur `/p/` (seule directive ajoutée à cette politique depuis l'étape 8, épinglée par `tests/serveur/application.test.ts`), la `VERSION` v3 du service worker (les polices entrent dans la coquille), le renommage de la classe `pastille` des facettes en `etiquette`, et `Echelle` remontée de `app/components/vitrine/` à `app/components/` parce que `ChoixNiveau` la partage. **Ce qui n'a pas changé** : les parcours, les URL, les loaders et les actions, le filtre de portée, la page d'un lien sans script (sa politique n'a toujours aucun `script-src`), la limite `style-src 'unsafe-inline'` de l'arbre authentifié et de `/p/`, et la revue de fuite — aucune surface nouvelle ne rend une donnée de la base. Spec : `docs/superpowers/specs/2026-09-15-releve-commun-design.md`.

145. **On n'entre plus par une variable d'environnement, mais par une invitation** (issue #62). La décision #130 avait explicitement écarté « un jeton d'invitation (un écran de plus, pour un cas qui n'est pas arrivé) » ; le cas est arrivé — laisser entrer quelques personnes choisies, chacune avec **son propre compte et ses propres propriétés**, ce que le modèle de données permet depuis l'étape 0 (`propriete.proprietaire_id`, et un utilisateur peut en posséder plusieurs). Ce qui a réellement décidé, ce n'est pas le nombre de comptes : c'est que **`AUTORISER_INSCRIPTION` était un mécanisme sans état**, et que tout ce qui était fragile chez elle en découlait. Posée, elle autorisait autant de comptes qu'on voulait (mesuré à six d'affilée) ; d'où le plafond dur à deux comptes écrit en code pour se protéger d'un `.env` jamais nettoyé — un pansement sur une variable, pas une porte. Une invitation porte son propre état : **un** compte, une date de fin obligatoire, révocable, et tracée en base avec qui l'a émise et quel compte en est sorti. Elle se borne donc toute seule, il n'y a plus rien à oublier dans un fichier, et la décision explicite du propriétaire passe du shell du VPS à un clic. **La table est calquée sur `partage`**, et pas par goût de la symétrie : c'est le même objet — un secret de 32 octets en base64url qui circule dans WhatsApp (`creerJeton`, partagé), des horodatages **datés et jamais supprimés** parce que la trace de qui on a laissé entrer est le point, et un refus **uniforme** (`MESSAGE_INSCRIPTION_FERMEE`) pour les cinq façons d'être inutilisable — absente, inconnue, expirée, révoquée, déjà consommée : on ne dit pas au visiteur si le jeton a existé, même raisonnement que « filtré = 404, jamais 403 ». **Deux détails portent plus qu'ils n'en ont l'air.** La consommation se fait dans la **même transaction** que l'insertion du compte, par un `UPDATE ... WHERE` qui répète les conditions de validité et dont on lit le nombre de lignes : c'est ça, et non le verrou consultatif, qui empêche deux inscriptions simultanées sur le même jeton de faire deux comptes. Et l'adresse est vérifiée **avant** que l'invitation soit consommée, sinon une faute de frappe sur une adresse déjà prise ferait perdre son lien à l'invité — un test l'épingle, parce que rien d'autre que l'ordre des deux lignes ne l'assure. **Ce qui est assumé** : tout compte connecté peut inviter, donc un invité peut inviter à son tour ; `invite_par_id` trace la chaîne et n'importe qui peut révoquer, ce qui est cohérent avec des comptes qui partagent la même instance, mais ce n'est pas une hiérarchie et ça n'en tiendra pas lieu. **Ce qui n'est pas dans le code et reste à dire de vive voix** : un invité vit chez vous — son compte est sur votre VPS, ses photos sur votre disque, et vous pouvez lire sa base. **Écran à part, et c'est le premier du genre** : `/proprietes/invitations` est le seul écran authentifié qui ne parle d'aucune propriété. Sous le préfixe `proprietes` malgré tout, parce que c'est `ACCUEIL` — donc aussi la portée du service worker : un `/invitations` à la racine serait dans l'arbre protégé tout en sortant de cette portée, sans que rien ne le signale.

146. **Une zone change d'étage, et sa descendance la suit.** L'écran de modification d'une zone ne savait que renommer et retyper : corriger « la buanderie est au sous-sol, pas au rez » demandait de la supprimer et de la recréer, donc de perdre ses sous-zones et de ressaisir les objets. Le rattachement (`niveau_id`) et la zone parente sont désormais modifiables. **Trois choses en découlent, et deux ne se voient pas depuis le formulaire.** (1) La descendance suit, sur tous ses niveaux, et ce n'est pas une commodité : `chargerArbreZones` groupe les zones par niveau AVANT de reconstruire l'arbre, donc un placard resté au rez sous une cuisine montée au premier ne s'afficherait pas « mal rangé » — il s'afficherait comme une zone racine du rez, détaché en silence. C'est exactement ce que `zoneParenteValide` interdit à la création. (2) Les contours (`zone_geom`) des plans qui ne couvrent plus la zone sont **effacés**, pas filtrés à la lecture : `chargerPolygonesDuPlan` ne filtre pas par couverture (son filtre est celui de la portée), donc `zoneDuPoint` continuerait de proposer « Cuisine » pour un point posé sur le plan du rez — c'est-à-dire de proposer d'écrire `element.zone_id` depuis une zone qui n'est plus là. Un contour se retrace en quelques clics ; une géométrie fausse qui dort en base est pire qu'absente. (3) **Aucun `element.zone_id` n'est réécrit** : les objets suivent leur zone sans qu'une ligne d'`element` bouge, donc la portée d'un lien de partage se recalcule seule et rien n'entre ni ne sort d'un partage à l'insu de quiconque — la même retenue que la décision #110. **Un piège NULL, et c'est le même que celui de `clausePortee`** : le `DELETE` des contours nie la couverture, et `z.niveau_id = p.niveau_id` vaut NULL — non `false` — dès que la zone devient extérieure ; sans `coalesce(…, false)`, le contour le plus certainement faux des trois était le seul à survivre. Un test le constate, et il échoue si on retire l'enveloppe. Nouveauté côté garde : le cycle (ranger une zone sous sa propre sous-zone) est refusé avant d'être écrit, parce qu'il ne planterait pas — `grouperParParent` ne traite comme racine que ce dont le parent est absent du lot, donc la branche disparaîtrait simplement de l'écran.

147. **Le type d'un objet devient facultatif** (`element.type_id` nullable, migration 0013). Le type était un péage : « le truc gris à côté du compteur » ne se consignait pas tant qu'on ne lui avait pas trouvé une case. Il redevient ce qu'il a toujours été — une **proposition** de champs et de niveau suggéré — et se pose plus tard, ou jamais. **Ce qui rend l'assouplissement sans danger, c'est ce qui ne bouge pas** : `element.zone_id` reste NOT NULL (règle non négociable #1), et c'est la zone plus `element.niveau` qui décident seuls de la portée d'un lien. Rien de ce que le type apporte n'entre dans une décision de visibilité, donc desserrer cette colonne n'ouvre rien. Le vrai risque du lot était l'inverse — un `JOIN type_element` interne oublié quelque part aurait **fermé** des fiches en silence — d'où le passage en `LEFT JOIN` des six lectures concernées et un test qui sert une fiche sans type à travers un vrai lien de partage. **Ce qui n'a pas eu besoin d'être touché** : `maj_recherche_element` (migration 0005), dont le `SELECT … INTO` ne ramène rien quand la colonne est nulle, laisse les variables à NULL et les voit sautées par `concat_ws` — le poids A du nom et le poids C de la zone restent, le poids B tombe à l'alias de la fiche seul, ce qui est exactement vrai. **Trois corollaires assumés.** Un objet sans type ne remonte plus sur les alias du catalogue (« robinet » ne trouve plus une vanne qu'on n'a pas typée) : c'est ce que « sans type » veut dire. Les facettes gardent un JOIN interne — une pastille a besoin d'un identifiant à cocher, « sans type » n'en est pas un — donc la somme des pastilles peut être inférieure au fonds, exactement comme pour `systeme`, nullable depuis toujours. Et retirer le type d'une fiche **n'efface pas ses `details`** : on masque, on n'efface jamais (règle #5), reposer le même type rend ses valeurs telles quelles. **La capture n'est pas touchée** : elle continue d'exiger un type, parce que c'est le geste des 30 secondes (règle #8) et que le type y est justement ce qu'on tape.

148. **Un lien de partage se corrige, sans que son jeton tourne** (migration 0014). Changer le plafond du plombier d'un cran demandait de révoquer son lien et d'en créer un autre : nouveau jeton, donc nouvelle adresse à lui renvoyer, alors que celle qu'il avait était en favori sur son téléphone. `majPartage` écrit le nom, le plafond, la portée et l'expiration, et **pas le jeton** — `SaisiePartage` ne le porte pas, donc le faire tourner n'est pas une case à ne pas cocher, c'est une erreur de compilation. La lecture du formulaire est **une seule** (`lireSaisiePartage`), partagée par la création et la correction : deux lectures dériveraient, et cette dérive-là s'appelle « le plafond n'est pas vérifié sur l'écran de modification ». **Le cas qui donne son nom au besoin** est l'expiration : prolonger le lien du locataire est exactement « garder le même lien », et un lien expiré se corrige donc. Un lien **révoqué**, non : c'est une trace — de ce qui a été ouvert, à qui, jusqu'à quand — et la réécrire serait réécrire l'histoire. Le refus vient du `revoque_le IS NULL` du `WHERE` et du nombre de lignes écrites, pas d'une lecture préalable, ce qui ferme du même coup la course entre une correction et une révocation simultanées. **Le corollaire est dit à l'écran plutôt que découvert** : ce qu'on change s'applique tout de suite à une adresse déjà distribuée — monter le plafond ouvre davantage à qui l'a déjà, le baisser referme sans prévenir personne — et l'aperçu est à un clic. **`partage.intervenant_id`** rattache un lien à quelqu'un du carnet : nullable (un voisin de passage n'a pas à y figurer), `ON DELETE SET NULL` et jamais CASCADE (retirer quelqu'un du carnet n'est pas révoquer son lien, même raisonnement que `revoque_le` daté et jamais supprimé). Il ne sort d'aucune requête de `/p/` et n'entre dans aucune `Portee` : ce qu'il permet, c'est de retrouver le lien du plombier **depuis sa fiche**. **Deux réglages opposés cohabitent donc sur cette fiche, et l'écran le dit en toutes lettres** : « Visibilité du nom et du métier » dit ce que les autres voient d'elle ; « Ce que cette personne peut voir » dit ce qu'elle voit de la maison. Écarté : ressusciter un lien révoqué — ce qui est coupé se remplace, c'est ce que la révocation voulait dire.

149. **Un coffre pour les codes, chiffré par le navigateur — jamais cherché, jamais partagé** (issue #73, migration 0015, note `.decisions/note-2026-09-17-chiffrement.md`). Le retour du 16 septembre demandait deux choses incompatibles — « chiffré sur le serveur » et « nous ne pouvons pas voir » — et une troisième qui n'était pas dite. **Écarté, le bout en bout global** : le serveur *lit* la base pour rendre ce que le produit vend — le `tsvector` de recherche est écrit par un trigger Postgres, une page `/p/` est rendue sans un octet de script avec le filtre de portée dans la requête, `sharp` redresse et réduit les photos — et une clé côté client transformerait un mot de passe oublié en maison perdue, quand on vient de livrer `npm run compte` (#66) comme seul recours. C'est un autre produit. **Écarté, un genre de champ `secret`** : la règle #6 (six genres, pas sept), et surtout le poids D de `maj_recherche_element`, qui indexe *toutes* les valeurs de `details` sans regarder `niveauMin` — un secret dans `details`, même chiffré, entrerait dans `element.recherche`. D'où une table `secret`, `element_id` NOT NULL comme `garantie`, hors de tout ce que le trigger lit. **Écarté, un mode « réinitialisable par le serveur »** pour la phrase oubliée : il ferait de nous des lecteurs possibles, et la page de confidentialité ne pourrait plus écrire « nous ne pouvons pas lire » sans un « sauf si » — deux modes, c'est zéro promesse. À la place, **une clé de données aléatoire enveloppée deux fois** (PBKDF2-SHA256 à 600 000 itérations puis AES-256-GCM, WebCrypto seul, aucune dépendance), par la phrase et par une **clé de secours** de 160 bits en base32, affichée une seule fois à la création et stockée nulle part : changer la phrase réenveloppe sans rechiffrer un seul secret (`md5` avant/après, test), la phrase oubliée s'ouvre par la clé de secours, les deux perdues se réparent en reprogrammant la serrure. Le bloc de vérification prévu dans la note est tombé : une mauvaise phrase échoue déjà sur l'étiquette GCM. **Ce qui porte le poids n'est pas le chiffrement, c'est ce qui ne sort pas** : les champs sensibles n'ont pas d'attribut `name` et ne sont dans aucun `<form>` — un `<input name="phrase">` partirait au serveur le jour où le JavaScript échoue avant l'hydratation — et l'envoi est un `fetcher.submit` construit avec le seul bloc ; aucune requête sous `app/lib/partage/` ni `app/routes/_partage/` ne nomme `coffre` ou `secret` (test statique) ; le balayage de toutes les colonnes de toutes les tables ne trouve ni phrase, ni clé de secours, ni valeur, `element.recherche` compris ; les droits nommés sur les fichiers restent à trois, un secret n'a pas de document ; la capture n'a pas changé d'un octet. La règle #9 du plan devient « aucun secret **en clair** », et `/confidentialite` dit ce qui est vrai après. **Assumé** : sans JavaScript la section dit qu'elle en a besoin (l'arbre authentifié en dépend déjà pour la capture et le plan, et c'est justement parce que le déchiffrement est *dans* le navigateur) ; la phrase est demandée à chaque geste et jamais gardée pour la session ; un secret pend à un objet et pas à une zone — deux points laissés ouverts dans la note, à rouvrir si la vraie maison le réclame.
...
- **Le tracé en cours vit dans le composant et meurt avec lui.** Changer de plan, recharger ou fermer l'onglet l'abandonne — les sommets sont des pourcentages de *cette* image-là, les garder les poserait n'importe où. Rien n'entre en base avant « Terminer le contour », ce qui est la lecture la plus simple de « tant que ce n'est pas fini, ce n'est pas enregistré » (même forme que la décision #89). Il survit en revanche à un **refus** du serveur depuis les finitions (décisions #121 et #122).
- **`plans/points` lance toujours ses 404**, quand `plans/contours` les rend. Ce n'est pas une incohérence oubliée : ce qu'une pose de point perdrait en remplaçant la page est le zoom et la position de la vue, pas une saisie — et le seul chemin qui écrit `element.zone_id` demande de toute façon un second geste. Le jour où cet écran porterait un état qu'un refus ne doit pas emporter, il prendra la même forme.
- **La retenue du tracé n'a pas de test de bout en bout.** `traceApres` est éprouvée seule et la route l'est côté serveur ; ce que fait le composant entre les deux a été vérifié au navigateur, à la main, et rien ne le rejoue. Le dépôt n'a ni DOM de test ni bibliothèque de rendu, et en ajouter une pour cet effet-ci serait un chantier sans rapport avec ce qu'il corrige.
...
- **Le chronométrage sur téléphone réel reste à faire**, sur le VPS et avec la vraie maison — c'est la moitié de l'étape 8 qui ne s'exécute pas depuis un poste de développement (voir [Mise en service](#mise-en-service)).
...
- **Les bornes de taille refusent sur la longueur ANNONCÉE, et refusent tout corps qui n'en annonce pas.** Un envoi qui n'annonce pas sa longueur ne peut être borné par personne — `request_body max_size` de Caddy ne borne qu'à la lecture, donc rien quand aucune route ne lit — et il est refusé en 411 des deux côtés plutôt que documenté (#33). Ce qui reste vrai et n'est pas vérifié depuis le dépôt : la part Caddy. Aucune suite ici ne lance Caddy ; seule la part Express est testée (`tests/serveur/`), et le refus côté Caddy attend le passage sur la machine cible.
- **Les sauvegardes ne quittent pas la machine tant que `SAUVEGARDE_DESTINATION` est vide**, et sur un VPS c'est plus grave que ça n'en a l'air : elles ne survivent alors ni à la perte de la VM, ni à un compte suspendu, ni à un incident chez l'hébergeur. Le script avertit à chaque passage, il ne refuse pas — la sauvegarde locale a réussi. La copie distante est un `rsync` **sans `--delete`**, ce qui protège d'une purge locale et **pas** d'une compromission : la clé qui écrit là-bas peut y écraser n'importe quel fichier. Une destination en ajout seul, ou une sauvegarde tirée depuis une autre machine, fermerait ce cas ; ni l'une ni l'autre n'est construite. La rétention de la copie distante est à faire là-bas.
...
- **`font-src 'self'` sur `/p/`.** La seule directive ajoutée à la politique de la page d'un lien depuis l'étape 8, pour deux fichiers servis d'ici : les polices du relevé, hachées dans `/assets/`. Sans elle, `default-src 'none'` les bloquerait et la page tomberait sur la police de secours — lisible, mais plus le même document que l'aperçu du propriétaire.
- **Ce qui n'est pas exécuté, c'est le VPS, plus l'architecture.** L'image de production est construite et lancée depuis le 12 septembre 2026 : Docker Desktop sur le PC Windows produit une image `linux/amd64`, c'est-à-dire l'architecture d'un VPS courant, et la répétition complète (compose, Caddy, migrations depuis une base vide, protections, sauvegarde et restauration) a tourné dessus. Reste vrai pour **arm64**, que certains hébergeurs proposent : `package-lock.json` porte les variantes `linux-x64` **et** `linux-arm64` des binaires natifs — en musl pour `sharp` et sa libvips, `argon2` et `rollup` ; `esbuild` publie un seul binaire Linux par architecture, sans variante musl —, et les trois images (`node:22-alpine`, `postgres:16-alpine`, `caddy:2-alpine`) sont publiées pour les deux. Rien de tout ça n'a été compilé en arm64. Et dans les deux cas, ce qui n'est pas fait est le `docker compose up` **sur la machine cible**.
- **Le build tourne sur la machine qui sert l'application**, et sa consommation n'est connue qu'en ordre de grandeur. `git pull && docker compose up -d --build` compile sur le VPS : **environ 90 secondes à froid** et **de l'ordre de 0,5 Go** de mémoire ajoutés au pic, mesurés le 12 septembre 2026 sur PC. Le chiffre de mémoire est **approximatif par construction** — échantillonnage de `MemAvailable` à ~1 Hz, coût de l'échantillonneur inclus, BuildKit hors de `docker stats` : il cadre le choix de l'offre, il ne le décide pas, et une pointe courte a pu passer entre deux mesures. Construire ailleurs demanderait un registre d'images ; c'est dans « En attente d'un besoin réel » du plan, déclencheur : le jour où le build échoue sur la VM retenue.
- **Le pays d'hébergement n'est pas décidé**, ni pour ni contre la Suisse, et le dépôt n'en dépend nulle part. La question est posée dans « À vérifier avant de s'appuyer dessus » du plan, avec son déclencheur : avant d'ouvrir l'inscription à un autre compte que celui du propriétaire (#130). Aucune affirmation juridique n'est faite ici.
- **La suggestion de niveau par type tombe à côté dans les cas mixtes.** Une cuve est suggérée « technique » parce qu'elle contient le plus souvent du fioul ou du gaz — une cuve d'eau de pluie est pourtant l'affaire du jardinier, et une prise extérieure suggérée « usage » peut alimenter l'installation d'alarme. Le type ne sait pas ce qu'on en fait : c'est pour ça qu'il **propose** et que la fiche tranche, et que la liste des objets montre le niveau de chacun. La limite est dans le mot « suggestion », pas dans la table.
- **La phrase du coffre perdue avec la clé de secours, c'est le coffre perdu.** Il n'y a rien côté serveur qui permette de le rouvrir, et c'est ce qui permet à `/confidentialite` de dire « nous ne pouvons pas lire vos codes » sans un « sauf si » : un second mode réinitialisable par le serveur a été écarté (décision #149). Le seul recours est de vider le coffre — un geste qui dit combien de secrets disparaissent — puis de ressaisir les codes après avoir reprogrammé ce qui doit l'être. La perte est dite à l'écran à la création, au moment où la clé de secours est montrée, une seule fois. Corollaire : `npm run compte` remet un mot de passe de compte, jamais une phrase de coffre.
