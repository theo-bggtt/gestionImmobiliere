...
115. **`jsonb_path_exists` plutôt qu'un cast `(s->>'x')::numeric`.** Sur une valeur textuelle, le cast lève une exception au lieu de rendre faux, et l'ordre d'évaluation des `AND` d'un `WHERE` n'est pas garanti en SQL : un garde de type placé avant ne protège pas d'un cast placé après. Le chemin JSON, lui, rend faux sur ce qui n'est pas un nombre. Vérifié en base sur les huit formes fautives avant d'écrire la migration, pas déduit — et les mêmes formes sont éprouvées par des `INSERT` directs dans `tests/plans/contours.test.ts`, qui contournent la route pour attaquer la contrainte elle-même.
...
117. **Un contour ne se trace que sur un plan qui couvre sa zone**, et la garde est une requête et non la liste que l'écran a affichée. `chargerZonesTracables` réutilise la règle de couverture de `clausePlanVisible` — plan d'étage : les zones du niveau ; plan de situation : les zones extérieures — plutôt que d'en écrire une seconde. Sans elle, la cuisine du rez se traçait sur le plan du sous-sol, et proposait ensuite une zone d'un autre niveau à tout objet posé dedans. La route de ressource est atteignable sans l'écran, donc la vérification vit dans le serveur : 404, sans distinguer « n'existe pas » de « n'est pas à vous ».
