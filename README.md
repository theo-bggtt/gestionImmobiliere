...
Identifiants de démonstration créés par `seed:exemple` : `demo@gestion-immobiliere.local` / `demo1234`. Ils sont publics, donc le script **refuse** de les créer sous `NODE_ENV=production` (ce que pose le conteneur) et dès que la base contient un compte qui n'est pas celui de la démonstration — pas un avertissement, un code de sortie 1 (décision #127, test dans `tests/scripts/`).
...
| Quoi | Où | Pourquoi là |
|---|---|---|
| TLS, renouvellement, redirection HTTP→HTTPS | Caddy | Il le fait seul, sans cron ni deuxième outil (décision #120) |
| `X-Forwarded-*` | Caddy → app | Caddy **remplace** ce qu'un client aurait écrit ; l'app se fie à **un** saut, jamais à `true` (décision #121) |
...
| Limite de débit | `server/application.js` | `/p/` et `POST /connexion`+`/inscription` seulement, jamais l'usage authentifié (décision #123) |
...
| Migrations | `app`, au démarrage | `scripts/migrate.mjs`, même rôle que l'application (décision #122) |
...
4. Charger le catalogue **depuis un poste de développement**, par un tunnel SSH vers le port 5432 que le compose ne publie que sur la boucle locale du Pi — l'image de production n'embarque pas `tsx`, et c'est voulu : le catalogue se charge une fois, ses alias se rafraîchissent rarement, et un outil de plus dans l'image ne vaut pas ce cas. `--env-file` laisse l'environnement gagner sur `.env`, donc :
   ...
   **Pas `seed:exemple`** : il refuse de toute façon dès qu'un compte réel existe (décision #127).
...
- cookie `Secure` derrière le proxy, constaté dans les en-têtes réels ;
...
- `Caddyfile`, `docker-compose.yml`, `Dockerfile` — la topologie d'une machine : Caddy devant, l'application et la base derrière. Voir [Mise en service](#mise-en-service).
