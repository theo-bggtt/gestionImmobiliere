...
**Le catalogue suggère, par type.** `type_element.niveau_suggere` (migration 0010) vaut 1 pour ce dont on a besoin pour habiter — prises, interrupteurs, luminaires, radiateurs, fenêtres, robinets, électroménager — et 2 pour ce qu'on ne touche que pour intervenir : tableau, disjoncteur, chaudière, chauffe-eau, compteurs, vanne d'arrêt, siphon, gaine, regard, fosse, pompe à chaleur, cuve, programmateur d'arrosage. **Aucun type ne suggère 0** : un niveau public se décide objet par objet. C'est le TYPE qui porte la suggestion et non la zone, parce que la portée d'un partage filtre déjà par zone et qu'un tableau électrique posé dans une cuisine doit rester masquable. Le re-seed rafraîchit `niveau_suggere` comme `alias`, jamais `champs` : c'est du vocabulaire de catalogue, qu'un propriétaire ne peut pas corriger sur un type système, et sa correction à lui vit sur la fiche.
